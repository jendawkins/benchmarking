...
def make_16s_tree(seq_config, sequences, sequence_data, save_path):
    ...
    os.chdir('..')
    filename = os.path.basename('utilities/' + phylo_outputs+'newick_tree_query_reads.nhx')
    dest = os.path.join(save_path+ '/tmp/', filename)
    shutil.move('utilities/' + phylo_outputs+'newick_tree_query_reads.nhx', dest)
    # os.chdir('..')
    print(os.getcwd())
...
def get_sequence_distance_matrix(sequence_data, sequence_tree):
    ...
    for seq1,seq2 in itertools.combinations(sequence_data.columns.values,2):
        ...
        dist[seq2][seq1] = d
        ct += 1
        if ct == np.int(np.round(perc * tot)):
...
def transform_func(data_train, data_test=None):
...
# def get_inchi(idType_id, met=None, i=None, outType='InChI Code'):
#     # type='InChIKey'
#     if i%100==0:
#         print(f'{i} metabolites left')
#     for idType, id in idType_id:
#         try:
#             if idType=='InChIKey':
#                 out = pcp.get_compounds(id, namespace='inchikey')[0]
#             elif idType=='InChi Code':
#                 out = pcp.get_compounds(id, namespace='inchi')[0]
#             else:
#                 out={}
#         except:
#             out={}
#             try:
#                 string = f"https://cts.fiehnlab.ucdavis.edu/rest/convert/{idType}/InChI Code/{id}"
#                 r = requests.get(string)
#                 out_tmp = r.json()
#             except:
#                 out_tmp = None
#             if out_tmp is None or len(out_tmp[0]['results']) == 0:
#                 out=None
#                 continue
#             out_tmp = out_tmp[0]['results'][0]
#
#
#             try:
#                 out = pcp.get_compounds(out_tmp, namespace='inchi')[0]
#             except:
#                 out=None
#
#         if out is not None:
#             try:
#                 if outType=='InChI Code':
#                     inchi=out.inchi
#                 if outType=='InChIKey':
#                     inchi=out.inchikey
#                 if outType=='SMILES':
#                     inchi=out.canonical_smiles
#                 return inchi, met
#             except:
#                 out=None
#                 continue
...
# def get_met_keys_old(metab_df, key_type='SMILES'):
#     metab_df.columns = [c.lower() for c in metab_df.columns.values]
#     res_dict = {}
#     mets_to_query = list(zip(np.arange(len(metab_df.index.values)), metab_df.index.values))
#     bad_mets = []
...
#             # print(f'No ID for {metabolite}, {i}')
#             continue
#         id_vec.append((id_types, metabolite, key_type))
#     if num_wo_id>0:
#         print(f'Warning: {num_wo_id} metabolites have no KEGG, HMDB, or INCHIKEY ID')
#     inchis_out = Parallel(n_jobs=8)(delayed(get_inchi)(idt_id,met=met,i=len(id_vec)-i,outType=k) for i,(idt_id,met,k) in enumerate(id_vec))
#
#     res_dict = {m: {key_type.lower(): inchikey} for inchikey, m in inchis_out}
#
#     res_df = pd.DataFrame(res_dict).T
#     print(f"{sum(~res_df[key_type.lower()].isna())} metabolites ID'd with {key_type}")
#     return metab_df.join(res_df,how='left')
